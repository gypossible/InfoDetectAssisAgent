...
- 默认实际搜索链路是：`Tavily News` 优先，`DuckDuckGo News` 兜底；如手动切换配置，也支持 `Bing News Search API` 和 `SerpAPI (Google News)`。
- 搜索来源属于公开网页/新闻检索来源，典型返回包括新闻媒体站点、财经网站、门户文章页、公开资讯页等；不直接覆盖需要登录或私有接口的微信、微博、小红书后台数据。
