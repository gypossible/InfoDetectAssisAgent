...
REQUIRED_SECTIONS = ["核心摘要", "昨日舆情综述", "重点主体异动分析", "潜在风险预警", "应对建议"]
...
class LLMReportGenerator:
    ...
    def _generate_with_llm(
        ...
        if self._needs_retry(first_pass):
            ...
            retry_input = (
                ...
                "确保不少于 1500 字，且完整包含：核心摘要、昨日舆情综述、重点主体异动分析、潜在风险预警、应对建议。\n\n"
                f"{first_pass}"
            )
    ...
    def _build_empty_report(run_time: datetime, start_time: datetime, end_time: datetime) -> str:
        ...
        return (
            f"# 每日舆情分析报告（{run_time.strftime('%Y-%m-%d')}）\n\n"
            "## 核心摘要\n"
            "过去 24 小时内，监测名单未检索到明确的新增新闻舆情样本，整体舆情保持平稳。"
            ...
            "## 潜在风险预警\n"
            "短期内风险相对可控，但仍需警惕突发事故、监管处罚、产品质量争议、经营传闻等事件导致舆情快速放大。"
